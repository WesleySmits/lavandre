...
export default class App {
    public initialize() {
        ...
        jQuery(document).on('elementor/popup/show', (event: any, id: any, instance: any) => {
            ...
            if (id === 6237) {
                const cart: any = Cart;
                cart.cartElement = document.getElementById('custom-cart') as HTMLUListElement;
                cart.updateCart();
            }

            if (id === 6324) {
                const cookieCheckbox: HTMLInputElement | null = document.getElementById('revoke-cookie-checkbox') as HTMLInputElement;
    ...
    private initializeDataPanelClicks() {
        ...
        for (let index = 0; index < panelLinks.length; index++) {
            ...
            element.addEventListener('click', (event: Event) => {
                ...
                const panelID: number = Number(element.dataset.panel);

                // If already logged in follow link
                if (element instanceof HTMLAnchorElement && panelID === 6188 && document.body.classList.contains('logged-in')) {
                    window.location.href = element.href;
                    return;
                }

                // @ts-ignore
                elementorProFrontend.modules.popup.showPopup({ id: panelID });
            });
        }
    }
    ...
    private initializeCurtain() {
        const toggles: HTMLButtonElement[] = Array.from(document.querySelectorAll('[data-curtain-toggle]'));
        ...
        for (let index = 0; index < toggles.length; index++) {
            ...
            const content: HTMLElement = document.querySelector(`[data-curtain-content="${toggle.dataset.curtainToggle}"]`) as HTMLElement;
            if (!content) {
                continue;
            }
