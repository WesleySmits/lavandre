...
describe('Product detail tests', () => {
    ...
    it('should have functioning +- buttons', () => {
        cy.get('input[name="quantity"]').should('exist').should('have.value', 1);
    ...
    it('should change the price when changing variants', () => {
        ...
        cy.get('.product-detail__price [data-product-price]')
            .first()
            .invoke('text')
            .then((text) => {
                price = text;
        ...
        cy.get('[name="attribute_pa_amount"][value="set-12-packs"]').should('exist').check();
